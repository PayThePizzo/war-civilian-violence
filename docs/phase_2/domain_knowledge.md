...
Each country stays in its geographic position, while colour and emphasis identify the fifty countries and territories included in the Conflict Index and distinguish their conflict severity. Areas outside the selection are muted, helping the highlighted regions stand out on a phone screen.
...
Country names line up on the left, and all bars use the same numerical scale. Bar length shows the number of registered refugees, end labels give exact values, and ordering the bars from largest to smallest makes the ranking easy to follow.
...
Each event with a location is assigned to a PRIO-GRID cell. These cells provide a common spatial unit, instead of grouping events by country, province, or named place. Filled cells or changes in intensity show where lethal organized violence was recorded during the stated period.
