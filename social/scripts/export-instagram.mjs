/**
 * Render both Instagram posts from the built dist/ output and save
 * deterministic 1080x1350 PNGs to social/output/. Serves dist/ over a plain
 * local static server (not vite preview) so the export never depends on a
 * dev-server transform pipeline, then drives each page with Playwright at
 * an exact viewport, waits for fonts + data + render to finish, and takes
 * one full-viewport screenshot per post. Run `npm run build` first (or via
 * `npm run export`, which does so automatically).
 */
...
// match instagram/post-*/index.html exactly.
const POSTS = [
  { path: '/instagram/post-01-temporal/index.html', output: 'instagram-01-temporal.png' },
  { path: '/instagram/post-02-actors/index.html', output: 'instagram-02-actors.png' },
];
...
async function exportPost(browser, baseUrl, post) {
  ...
  await page.screenshot({ path: outputPath, clip: { x: 0, y: 0, ...VIEWPORT } });
  await page.close();
...
async function main() {
  ...
  console.log(`Exported ${POSTS.length} Instagram graphics to ${outputDir}`);
}
